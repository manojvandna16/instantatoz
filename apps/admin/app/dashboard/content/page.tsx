...
import { FileText, Plus, Edit3, Check, X, Eye, EyeOff, Save } from 'lucide-react';
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">Manage app and website content, descriptions, and copy.</p>
      </div>

      <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4">
        <p className="text-xs text-amber-400">
          ⚠️ Content shown here is managed locally in this admin session. To persist content changes, connect this to a Firestore <code className="bg-black/30 px-1 rounded">app_content</code> collection or your CMS.
        </p>
      </div>
              ...
              <div className="flex items-center gap-2">
                ...
                {editing !== section.id ? (
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => setEditing(null)} className="text-xs text-gray-400 hover:text-white bg-gray-800 px-2 py-1.5 rounded-lg">
                      <X className="w-3 h-3" />
                    </button>
                    <button onClick={() => saveEdit(section.id)} className="flex items-center gap-1 text-xs text-white bg-blue-600 hover:bg-blue-700 px-3 py-1.5 rounded-lg">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <button className="flex items-center gap-1.5 text-xs text-blue-400 hover:text-blue-300 bg-blue-600/10 hover:bg-blue-600/20 px-3 py-1.5 rounded-lg">
        ...
        <div className="space-y-2">
          {[
            ...
            { q: 'What payment methods are accepted?', a: 'UPI, credit/debit cards, and net banking via Razorpay. All transactions are secure.' },
            { q: 'What if I am not satisfied with the service?', a: 'Contact support within 24 hours of job completion. We review all complaints and offer refunds where applicable.' },
          ].map((faq, i) => (
