          ...
          <div key={worker.uid} className="bg-gray-900 border border-amber-800/30 rounded-xl p-5">
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button 
                  disabled={processingId === worker.uid}
                  onClick={() => handleVerify(worker.uid, 'NEEDS_MORE_INFO')}
                  className="flex items-center gap-1.5 px-3 py-2 bg-blue-600/20 hover:bg-blue-600/40 text-blue-400 text-xs font-medium rounded-lg transition-colors border border-blue-600/30 disabled:opacity-50">
                  <Info className="w-3.5 h-3.5" /> Need Info
                </button>
                <button 
                  disabled={processingId === worker.uid}
                  onClick={() => handleVerify(worker.uid, 'REJECTED')}
                  className="flex items-center gap-1.5 px-3 py-2 bg-red-600/20 hover:bg-red-600/40 text-red-400 text-xs font-medium rounded-lg transition-colors border border-red-600/30 disabled:opacity-50">
                  <XCircle className="w-3.5 h-3.5" /> Reject
                </button>
                <button 
                  disabled={processingId === worker.uid}
                  onClick={() => handleVerify(worker.uid, 'APPROVED')}
                  className="flex items-center gap-1.5 px-3 py-2 bg-green-600 hover:bg-green-700 text-white text-xs font-semibold rounded-lg transition-colors shadow-lg shadow-green-600/20 disabled:opacity-50">
                  {processingId === worker.uid ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <CheckCircle className="w-3.5 h-3.5" />} Approve
                </button>
              </div>
            </div>
